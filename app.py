...
# Custom Styling strictly matching original Screenshot 2
st.markdown("""
<style>
...
if 'selected_record' not in st.session_state:
    st.session_state['selected_record'] = paimana_df.iloc[0].to_dict()
...
selected_subdiv = st.sidebar.selectbox("3. Subdivision (101 Subdivisions)", subdivisions, index=0)

blocks = geo_hierarchy.get(selected_district, {}).get(selected_subdiv, ["Chakia"])
...
with col_sec1:
    ...
    current_proj_name = st.session_state['selected_record'].get('Project_Name', project_options[0])
    curr_idx = project_options.index(current_proj_name) if current_proj_name in project_options else 0
    
    selected_inspect = st.selectbox("Select Construction Work to Inspect:", project_options, index=curr_idx)
    active_row = next((r for r in matched_projects if str(r["Project_Name"]) == selected_inspect), matched_projects[0])

    # White Project Card as in Screenshot 2
    st.markdown(f"""
    <div class="project-card-white">
        ...
        <div class="contractor-text">🏗️ {active_row.get('Contractor_Name', 'Tata Projects Ltd.')}</div>
    </div>
    """, unsafe_allow_html=True)
    
    # 2-Column Metadata Bullets
    b1, b2 = st.columns(2)
    with b1:
...
with col_sec2:
    ...
    with s2_col1:
        inp_cost = st.number_input("Cost (₹ Cr)", value=float(rec.get('Original_Cost_Cr', 341.56)), key="inp_cost")
        inp_duration = st.number_input("Original Duration (Months)", value=int(rec.get('Original_Duration', 27)), key="inp_dur")
        inp_elapsed = st.number_input("Elapsed Time (Months)", value=int(rec.get('Elapsed_Months', 13)), key="inp_elap")
        inp_spend = st.number_input("Cumulative Spend (₹ Cr)", value=float(rec.get('Cumulative_Spend_Cr', 200.56)), key="inp_sp")
    with s2_col2:
        inp_phys = st.slider("Physical Progress (%)", 0.0, 100.0, float(rec.get('Physical_Progress_Pct', 40.80)), key="sl_phys")
        ...
        inp_land = st.slider("Local Land Risk (1-10)", 1.0, 10.0, float(rec.get('Land_Risk_Score', 7.70)), key="sl_land")
        inp_wpi = st.slider("WPI Material Inflation Index", 90.0, 140.0, float(rec.get('WPI_Inflation_Index', 116.50)), key="sl_wpi")
...
with t_scurve:
    time_pts = np.linspace(0, inp_duration + max(12, int(pred_delay_months) + 6), 20)
    planned_s = 100 / (1 + np.exp(-0.15 * (time_pts - (inp_duration/2))))
    actual_pts = np.linspace(0, inp_elapsed, 10)
    actual_s = np.linspace(0, inp_phys, 10)
    forecast_pts = np.linspace(inp_elapsed, inp_duration + pred_delay_months, 10)
